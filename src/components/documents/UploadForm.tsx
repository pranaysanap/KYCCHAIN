...
import { verifyDocumentWithAI } from "../../services/aiService";
...
const UploadForm: React.FC<UploadFormProps> = ({ onUploadSuccess }) => {
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    setIsVerified(false); // Reset verification status on new file
    setSuccessMessage("");
  ...
  const handleVerify = async () => {
    ...
    try {
      const result = await verifyDocumentWithAI(selectedFile, documentType);

      if (result.isValid) {
        setIsVerified(true);
        setSuccessMessage(result.message || "AI Verification Successful: Valid Document Detected");
      } else {
        setIsVerified(false);
        setError(result.message || "AI Verification Failed: Document invalid or unclear");
      }
    } catch (err) {
        ...
        <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4 mt-6">
          {!isVerified ? (
            <Button
              ...
              icon={isVerifying ? <div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"></div> : <Bot className="w-4 h-4" />}
            >
              {isVerifying ? "Verifying with AI..." : "Verify Document with AI"}
            </Button>
          ) : (
            <Button
              variant="primary"
